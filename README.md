...
**問題**: サインインシートが表示されるが認証に失敗する

**解決方法**:
1. `GoogleService-Info.plist` がプロジェクトに含まれているか確認
2. `Info.plist` の `REVERSED_CLIENT_ID` が正しく設定されているか確認
3. Firebase Console で Google Sign-In プロバイダーが有効になっているか確認
4. iOS Simulator ではなく実機でテストしてみる
...
**解決方法**:
パッケージは `FirebaseAuthTokenProvider` を使って自動的にトークンをリフレッシュします。環境値が正しく設定されているか確認してください：

```swift
.environment(\.authTokenProvider, FirebaseAuthTokenProvider())
```

### ビルドエラー

**問題**: `GoogleService-Info.plist` が見つからない

**解決方法**:
1. ファイルが Xcode プロジェクトに追加されているか確認
2. Target Membership でアプリのターゲットにチェックが入っているか確認
3. ファイルがコピーされていることを確認（参照ではなく）
