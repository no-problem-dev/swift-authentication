...
vendor 非依存な ``Authentication/AuthCredential`` を生成します。
生成した資格情報は `AuthenticationFirebase` の `FirebaseAuthenticator` など、
``Authentication/Authenticator`` を実装した交換層に渡されます。
...
このモジュールが提供する `AuthCredential.apple(idToken:rawNonce:fullName:)` ファクトリを使用します。
